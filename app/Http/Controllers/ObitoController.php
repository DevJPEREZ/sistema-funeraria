...
class ObitoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $falecido = $this->getFalecido($request->falecido);

        try{
            $falecidoID=$this->falecido->create([
                'cpf' => $falecido->cpf,
                'nome' => $falecido->nome
            ])->id;
            $this->obitos->create([
    ...
    public function checkFalecido($cpf, $update)
    {
        ...
        if(!$update && $falecido && $this->falecido->where('cpf', $cpf)->exists()){
            $falecido = false;
        }

        if($update && $this->falecido->where('cpf', $cpf)->exists()){
            $falecido = true;
        }
    ...
    public function getFalecido($cpf)
    {

        $falecido = $this->titular->where('cpf', $cpf)->first();

        if(!$falecido){
            $falecido = $this->dependente->where('cpf', $cpf)->first();
        }
