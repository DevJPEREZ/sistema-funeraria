...
class ContaController extends Controller
{
    ...
    public function __construct(){
        ...
        $this->falecido = new FalecidoModel;
    }
    ...
    public function show($id)
    {
        ...
        $falecidos = new FalecidoModel;
        
        $pessoas = [];
        if(!$falecidos->where('cpf',$titular->cpf)->exists()){
            array_push($pessoas,['cpf' => $titular->cpf, 'nome' => $titular->nome]);
        }

        if($dependentes){
            foreach($dependentes as $dados){
                if(!$falecidos->where('cpf',$dados->cpf)->exists()){
    ...
    public function edit($id)
    {
        ...
        $dependentes = $conta->titular->dependentes;        
        foreach($dependentes as $key => $dependente){
            if($this->falecido->where('cpf', $dependente->cpf)->exists()){
                unset($dependentes[$key]);
            }
        }
        $estados = $this->estados->all();
        $status = $this->status->all()->sortby('status');
        ...
        $planos = $this->planos->planosAtivos()->sortby('plano');
        $parentescos = $this->parentescos->all()->sortby('parentesco');
        return view('contas.editar', compact('conta','dependentes','estados','status','civil','planos','parentescos'));
    }
